...
Alternatively can create seven_tensors and three_tensors directly from MLUDatasets:
...
##  @. is generating "rand.(20)", which is a vector of one element of vectors of 20 random numbers 
## speed = @. rand(20) * 3 .+ (0.75 * (time - 9.5)^2 + 1)

speed = @. $rand(20) + 0.75 * (time - 9.5)^2 + 1
speed = @. rand(20)[1] + 0.75 * (time - 9.5)^2 + 1
...
for i in range(1, 4)
    push!(plts, show_preds(apply_step!(params; lr=0.01, prn=false)))
end
...
lr = 0.01  ## how to adjust learning rate? takes a lot of time to converge
...
for epoch in 1:20
    for dd in dl
        xb, yb = reformat_dl(dd)
        loss, grads = Flux.withgradient(linear_flux) do m
