...
```{julia}
train_data, val_data = splitobs(data; at=0.5, shuffle=true)
```
...
finetune!(learner, 4)
savetaskmodel("artifacts/c5_resnet.jld2", task, learner, force=true)
```

```{julia}
task, model = loadtaskmodel("artifacts/c5_resnet.jld2")

samples = [getobs(data, i) for i in rand(1:numobs(val_data), 9)]
images = [sample[1] for sample in samples]
_labels = [sample[2] for sample in samples]

preds = predictbatch(task, model, images)
```
