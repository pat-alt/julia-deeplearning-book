...
```{julia}
train_data, val_data = splitobs(data; at=0.75, shuffle=true)
```
...
Note that we have decreased the floating point precision here. This increases training speed at the cost of precision as discussed later in the book chapter. 
:::
...
```{julia}
showbatch(ShowText(), task, first(train_dl))
```
